...
- **Idempotency** via a guest marker file whose content is a hash of normalized extensions + settings. Skip when hash matches; re-apply when config drifts without rebuilding the container. MUST NOT blindly re-apply every postAttach / every open
