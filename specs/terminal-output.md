...
Host terminal presentation for `adevcontainer`: stream roles (stderr progress/warn/tool vs stdout JSON/digests), product severity prefixes, internal tool framing, color/monochrome policy, phase section spacing, clone populate live stream, and post-success human layout. Complements lifecycle/Features domain specs for progress text families.
