...
- [release-distribution.md](conventions/release-distribution.md) — release, distribution, GitHub Actions (ci.yml / release.yml), macos-26 arm64, version inject (`Version.swift`, tag source of truth), tarball + sha256, Homebrew tap live (`wcgomes/tap`, `brew install adevcontainer`, first release v0.1.0), in-repo + tap formula sha sync, curl/tar fallback, COPYFILE_DISABLE, notarize deferred, prerelease `-` flag
