...
- [release-distribution.md](conventions/release-distribution.md) — release, distribution, maintainer process (`main` ≠ release; ship only via `git tag vX.Y.Z` / dispatch; non-prerelease auto Homebrew bump via `HOMEBREW_TAP_TOKEN`), GitHub repo `wcgomes/apple-dev-containers` (ex-`dev-containerization`, 301s), GitHub Actions (ci.yml / release.yml), macos-26 arm64, version inject (`Version.swift`, tag source of truth), tarball + sha256, Homebrew tap live (`wcgomes/tap`, `brew install adevcontainer`), `scripts/render-homebrew-formula.sh`, tap formula in-job + bot PR `brew-bump-vX.Y.Z` for in-repo formula, prereleases skip brew, missing token fails non-prerelease, curl/tar fallback, COPYFILE_DISABLE, notarize deferred, branch protection on main
