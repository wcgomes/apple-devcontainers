...
On `up`/`clone`/`rebuild` create paths when nested `build` is admitted:

- Product tag via `container build` **before** Features and create. Features `FROM` that tag (not config `image`).
- Clone: dockerfile and context stay inside the config-file directory.
- `up`/`clone` reuse existing product Dockerfile and Features derived tags.
- `rebuild` always rebuilds the Dockerfile image. Nested `build` + Features also rebuilds the Features derived image (same tag names allowed). Image-based Features rebuild still reuses.
- Volume/clone-origin `rebuild` stages the config-file directory (file siblings at that level) — not the workspace tree. JSON-only volume resolve is not enough.

Contract: [`specs/core.md`](../../specs/core.md) + [`specs/features.md`](../../specs/features.md) + [`specs/managed-lifecycle.md`](../../specs/managed-lifecycle.md); archive [`20260911-dockerfile-build`](../../specs/changes/archive/20260911-dockerfile-build/).

### `${devcontainerId}` (deferred expand)

Resource identity stem `adev-{base}-{hash12}` (empty base → `adev-{hash12}`). Base = workspace/repo basename, **never** config `name`. Same hash material as `*-ws`. **Not** the DNS create name. Features (e.g. shell-history) and config may embed it in **volume mount `source`** (`${devcontainerId}-shellhistory`) and **`containerEnv`** values.

| Phase | Behavior |
|-------|----------|
| Resolve (`VariableSubstitutor`) | Admit token. Expand only when `SubstitutionContext.devcontainerId` is set; otherwise **leave literal** `${devcontainerId}` (stem often unknown until after Features merge / mode-specific identity). |
...
- **Sanitize:** lowercase; non-`[a-z0-9-]` → `-`; **collapse consecutive hyphens** (`-{2,}` → `-`); trim leading/trailing hyphens. Shared algorithm; **different inputs.** Config `name` → create name / DNS only. Workspace/repo basename → human base, `${devcontainerId}` stem, `*-ws`, Features tags. Example: folder `C# (.NET)` → `c-net` (not `c----net`) so Features tags stay valid (`adev-c-net:{hash12}`).
- **Create name** (`create --name` = Apple id / DNS hostname): sanitize(`name`) if present and non-empty after trim; else mode-specific fallback (bind: workspace folder basename; volume: git URL repo basename, not the temp checkout). Clip ≤63 and re-trim hyphens. Human ID + DNS only. **No `adev-` prefix, no identity hash.** Empty after sanitize → structured fail asking for a DNS-safe `name` (no invented `adev-{hash12}`). Example: `"name": "My App"` in folder `foo` → DNS `my-app`.
- **`${devcontainerId}`** (resource stem): `adev-{base}-{hash12}` (empty base → `adev-{hash12}`). Base = workspace/repo basename, **never** config `name`. Same `base`+`hash12` as `*-ws`. Rebuild rename keeps stem-keyed volumes. Example: folder `foo` + `"name": "My App"` → stem `adev-foo-{hash12}`.
- **Human base** (`sanitizeBase`, ~20 clip of workspace folder basename / volume-mode repo basename): stem + hashed sidecars. **Never config `name`.**
- **Sidecar `hash12`** (not appended to the create name):
  - Bind: workspace path + config path. Reuse/occupancy key off labels, not this hash in the create name. Same material as the bind stem.
  - Volume: normalized git URL + config relpath (not temp checkout path). Same material as the volume stem and `*-ws`.
- **Workspace volume (volume-mode):** `{stem}-ws` = `adev-{base}-{hash12}-ws`.
- **Features derived image tag:** `adev-{base}:{hash12}` — `{base}` is the workspace/repo human base, **never** config `name`; `{hash12}` is the content hash of base image + features + **`recipeVersion`** (epoch string in `DerivedImageTag`); empty base → `adevcontainer:{hash12}`. No `adevcontainer/features:` prefix and no `/features` path segment. Config `image` without nested `build` or Features is left as written. Nested `build` uses a distinct product Dockerfile tag; Features `FROM` that tag when both are present. Rebuild rename keeps the same tag. Tag validity depends on sanitize collapse (above). **Bump `recipeVersion` whenever install-Dockerfile semantics change** so cached local derived images are not reused forever; current epoch **`"7"`** = COPY/RUN/`/tmp` identity paths use `feature-<slug>` (sanitized metadata `id` else last-segment ref; first keeps slug, collisions append `-<index>` then `-<index>-N`). Was `"6"` = derived LABEL unions base-image + feature lifecycle metadata. Was `"5"` = features-only LABEL + chmod-before-install + install-as-root then restore base USER + metadata `containerEnv` as Dockerfile `ENV` before install RUN (options + user keys on RUN prefix). Was `"4"` = same install layers but `containerEnv` on RUN prefix (single-quoted `PATH` could wipe system PATH). User-literal volume `source` strings are not rewritten.

**Create-name occupancy.** Same workspace = bind `local_folder`+`config_file`, or volume `git_url`+config identity. Leftover same-workspace different-name wins over a foreign occupant of the desired name on `up`/`clone` (delete-hint). `rebuild` applies the live computed create name (user `name` change and old `adev-*` migrate); selected leftover is not a delete-hint. Foreign occupant of the new name: do not delete selected; TTY Y/n + new name (persist into live config). `start` does not classify occupancy; idle containers are not renamed.

| Occupant | Outcome |
|----------|---------|
| Same workspace, same create name | `up`: existing reuse / start-stopped / `config_hash_mismatch`. `clone`: fail-closed naming the occupant. No rename-to-duplicate. |
| Same workspace, different name (incl. leftover `adev-*`) | `up`/`clone`: fail + delete-hint. No foreign picker. `rebuild`: selected leftover is not a delete-hint — recreate under the live computed name. |
| Desired name taken by another workspace or unmanaged | **Foreign occupant.** TTY (no `--json`): warn + ask whether to change the name, then prompt for the new name on stderr (not QUIET-silenced). Persist the sanitized name into config `name` (`up` host config / `clone` retained checkout; clone overlay after populate; `rebuild` live config — selected not deleted). Empty/over-63 or still colliding → re-prompt. Decline/EOF → original name-in-use error. No recovery editor, no suffix picker. Non-TTY/`--json`: fail + edit/retry hint; never prompt. Occupant untouched. |
| None | Create under the desired name. |
...
Apple named volumes mount **root:root** ([gaps](../domain/devcontainer-apple-gaps.md#named-volume-ownership-rootroot)). `WorkspaceOwnership` makes eligible volumes writable by a non-root connection user before create-path hooks; it is a no-op for root/unset users and never chowns bind mounts, readonly volumes, or empty targets.
...
| Workspace volume (`adev-*-ws`) | Separate `ensureWorkspaceWritableByRemoteUser` on `clone`; on volume-mode `rebuild` only when the effective connection user **differs** from stamped `remote_user` (data already belongs to that user otherwise) |
| Workspace parents (bind-mode create paths) | `ensureWorkspaceParentsWritableByRemoteUser`: rootfs ancestors of the workspace folder only — never the folder itself (host bind target). Runs on fresh `up` and `rebuild` (both modes); not on reuse/start-stopped, bare `start`, or `clone` |

**Normal path (`CHOWN` available):** after the main container starts, one root exec creates each target, recursively chowns it, then non-recursively chowns intermediate parents up to the system-top break list (`/`, `/home`, `/Users`, `/var`, …). The parents-only variant creates the rootfs workspace path and chowns ancestors only, never the bind target itself.

**Capability-preserving path (`CHOWN` unavailable):** `--cap-drop=ALL` or `--cap-drop=CHOWN` selects an ephemeral ownership helper unless `--cap-add=CHOWN` restores the capability. Before the main container starts, the helper uses the same image and platform, mounts only eligible read-write named volumes at private helper targets, and runs as root with `ALL` dropped plus only `CHOWN` and `DAC_READ_SEARCH` added. The main container keeps exactly its requested capabilities. The helper resolves the connection user's UID/GID from the shared image, rejects non-numeric results, verifies every private target in `/proc/self/mountinfo`, and chowns by numeric `UID:GID`; original container target strings never enter helper argv or script. Helper deletion is attempted after every successful helper create and completes before main start; operation and cleanup failures are surfaced.

The helper cannot repair the main container's private rootfs. After main start, the parents path is therefore checked as the connection user; inaccessible parents fail explicitly and instruct the user to restore `CAP_CHOWN` to the main container. Any ownership, helper, cleanup, or parents-check failure stops the create path before hooks. `up` and `clone` delete the new container (`clone` also deletes `*-ws`); `rebuild` enters post-delete recovery when eligible, otherwise deletes the replacement and returns the error.
...
### Git credential seeding (create paths)

After ownership, before create-path hooks, `up` fresh create (bind) and `rebuild` replacement (both modes) seed the resolved connection user's username-agnostic guest Git credential helper (`GuestGitCredentialSeed`). Remote source: bind — host `git -C <hostWorkspace> remote -v`, unique **HTTPS fetch** URLs only; rebuild volume — stamped `devcontainer.git_url`. HTTPS credentials are acquired on the host via the shared fill contract (`GIT_TERMINAL_PROMPT=0`; `ADEVCONTAINER_GIT_TOKEN` / `gh auth token` fallbacks), then one exec as the effective connection user resolves that user's HOME from passwd data rather than inherited HOME, writes `$HOME/.adevcontainer/git-credential-adev` mode 0700, appends it with `git config --global --add credential.helper <absolute-path>`, and feeds one pathless entry per unique (protocol, host, username) to `git credential approve` over stdin. The helper's private store is `$HOME/.adevcontainer/git-credentials` mode 0600; `get` matches protocol+host while ignoring the queried username and returns that username (or the stored username when the query has none) with the stored password, so sibling repos on the same host are covered. Seeded credential material enters the container via stdin only, never argv or env; errors redact URL userinfo and credential material. Silent skips (no exec, no warning): host git missing, non-repo or no fetch remotes, fill nil (public remotes stay anonymous), stamped `git_url` missing. Failure **soft-fails** — stderr warning, create path continues, no delete, no bring-up recovery; hook failure stays the hook's own failure. Non-create paths (`up` reuse, `up` start-stopped, bare `start`) never seed. `clone` is separate — its populate uses one-shot auth followed by guest `credential.helper store` + `git credential approve`; clone does not invoke this seeding path.

**Test seam:** command tests driving `up` fresh-create or `rebuild` replacement that install `RecordingHostProcessRunner` via `LifecycleRunner.hostProcessRunnerOverride` must answer or tolerate the seeding probe call (`git -C <ws> remote -v`), same as the adapted initialize-command tests.
...
2. Host git: sparse/shallow **config-only** fetch into a temp dir — cone is `.devcontainer/` (`sparse-checkout set --cone .devcontainer`; nested `features/<id>` present; cone also keeps root files such as `.devcontainer.json`; other repo-root directories are not materialized). Nested `build` dockerfile/context stay inside the config-file directory (root sibling `Dockerfile` + context `"."` is in-directory). Features load via `DefaultFeatureFetcher` on that checkout. Auth = host helpers/SSH agent; git argv puts `--` before the URL. Identity/labels use **normalized** URL (`scheme://` userinfo stripped); host git still gets the **original** URL.
...
6. Ensure workspace volume (`adev-{base}-{hash12}-ws`); delete+create if present. Occupancy of the create name: same-workspace same-name fail-closed (no silent reuse); leftover different name → delete-hint; foreign → TTY ask-to-rename then prompt for new name (persist into config `name`). See [occupancy](#deterministic-names-and-labels).
7. Create the volume-mode container (workspace = named volume; labels as above). Features runner runs when features are non-empty after step 5.
...
8. **Ownership + start:** when `CHOWN` is unavailable, make the workspace volume + config `type=volume` targets writable through the scoped helper and then start the main container; otherwise start the main container first and use the normal ownership path (`WorkspaceOwnership`; hard-fail + delete container/`*-ws` on failure) — see [Named volumes / ownership](#named-volumes-ensure--reuse--ownership).
...
`up` remains bind-mode host workspace only (no auto git Feature). The global HOME is container-scoped, not the persistence mechanism; workspace-local `.git/config` survives through the bind path or workspace volume. Detail: [architecture.md](../architecture.md); contract [`specs/clone.md`](../../specs/clone.md). Eligible clone recovery: [Bring-up recovery](#bring-up-recovery-bringuprecovery).
...
| Path | Behavior |
|------|----------|
| `up` create | Fresh bind-mode create when occupancy is none (desired name free, no same-workspace leftover) |
...
| `rebuild` | **Forced rebuild** (landed; archive [`20260810-rebuild`](../../specs/changes/archive/20260810-rebuild/)): read current stamped config **before** any delete and capture the existing workspace repository’s local author identity (bind: host workspace; volume: old container workspace); hostRequirements + Features; then container-only delete old → create under the **live computed** name (user `name` change and old `adev-*` migrate). Reuse stem-keyed workspace `adev-*-ws` and `${devcontainerId}` volumes (ensureVolume, never delete/replace workspace volume; no re-clone). After ownership (pre-start helper or post-start normal path) and credential-seeding work, synchronize a complete pair to the replacement connection user’s global Git config before create-path hooks; missing/incomplete → no invented value; host Git config and other container users remain unchanged. Foreign occupant of the new name: do not delete selected; TTY Y/n + new name (persist into live config). `${devcontainerId}` expands to the resource stem (stable across create-name change). Volume/clone-origin with no host workspace still runs host `initializeCommand` from a temp guest-config root (not skip; see [Lifecycle](#lifecycle-execution-hook-matrix)). Pre-delete failure leaves old container untouched. Optional `--skip-pull` / `--vscode` / `--json`. Volume `rebuild`: if any admitted ref is local-path, stage guest `.devcontainer` (and root `.devcontainer.json` when that is the config) via `exec tar` (not `container cp`) onto a host temp **before** Features/delete, then `DefaultFeatureFetcher`; fail structured pre-delete (old container kept); clean temp. Mixed local+OCI uses `DefaultFeatureFetcher`. OCI-only volume may still use `OCIFeatureClient`. Ownership is mandatory: volume-mode workspace only if connection user ≠ stamped `remote_user`, config volumes always, and rootfs parents always checked/fixed; failure enters post-delete recovery when eligible or deletes the replacement — see [ownership](#named-volumes-ensure--reuse--ownership). Hard post-delete recovery mode-split (TTY `Open the recovery editor now? [Y/n]` default Y; decline/EOF retain; named retry skips prompt; README + CLI help document UX): [gaps — Failed rebuild recovery](../domain/devcontainer-apple-gaps.md#failed-rebuild-recovery-mode-split). Bring-up (`up`/`clone`/`start`) is a separate primitive — [below](#bring-up-recovery-bringuprecovery) |

Nested `build` on `rebuild` (Dockerfile + Features rebuild, config-dir staging): [Dockerfile image](#dockerfile-image-nested-build).
...
| Config `image` or stamped product Dockerfile tag | Yes |
...
On `up`/`clone`/`rebuild` when `features` is non-empty after resolve (and after clone git-ensure). Code: `Sources/ADevContainerLib/Features/`.

| Step | Behavior |
|------|----------|
| Admit | Feature ref → options map; **OCI** (`ghcr.io/…/node:1`) and **local path** (`./…`, `../…`, absolute `/…`, `file://…`). Relative refs resolve from the config-file directory, then `workspace/.devcontainer/`, then workspace root — official beside-json `"./my-feature"` and `./features/<id>` (→ `<ws>/.devcontainer/features/<id>`) both load. Absolute/`file://` unchanged. Declaration key-sorted for stable ties |
...
| Fetch / load | **Local path:** validate package (`devcontainer-feature.json` + `install.sh`) and copy into feature cache via `DefaultFeatureFetcher` / `LocalFeatureLoader`. Relative refs resolve from config-dir / `.devcontainer` / workspace root. Bind `up`/`rebuild` and `clone` use `DefaultFeatureFetcher` on the host workspace or clone checkout (`clone` sparse cone is `.devcontainer/`, so nested `features/<id>` is present). Volume `rebuild`: if any admitted ref is local-path, stage guest `.devcontainer` (and root `.devcontainer.json` when that is the config) via `exec tar` (not `container cp`) onto a host temp **before** Features/delete, then `DefaultFeatureFetcher`; fail structured pre-delete (old container kept); clean temp. Mixed local+OCI uses `DefaultFeatureFetcher` (splits). OCI-only volume `rebuild` may still use `OCIFeatureClient`. **OCI:** embedded HTTPS client (`OCIFeatureClient`) — **not** `container image pull`, ORAS, or Node |
| Order | `dependsOn` / `installsAfter` topo-sort (id last-segment match so `./x/sample-a` satisfies `…/sample-a:1`); cycle → structured error |
| Build | Generate Dockerfile (`FeatureDockerfileGenerator`): per feature `COPY` package as `feature-<slug>` → `/tmp/adev-feature-<slug>` (slug = sanitized metadata `id` else last-segment ref; first keeps slug, collisions append `-<index>` then `-<index>-N`; `# Feature: <full reference>` comments remain); emit metadata **`containerEnv` as Dockerfile `ENV`** (so `$PATH`/`$VAR` expand — not single-quoted RUN env); then **`RUN chmod -R 0755 /tmp/adev-feature-<slug> && … ./install.sh` as root** with **options + `_REMOTE_USER` / `_CONTAINER_USER` on the RUN prefix only** (base-image USER when config remote/container user empty — not hardcoded `vscode`/`node`; RUN prefix overwrites ENV on key collision). After all features, **restore base image OCI `USER`**. Recursive +x avoids exit **126** on bare-path lifecycle hooks copied 0644 from OCI. `container build --platform` host-native (`linux/arm64` on Apple Silicon) via `AppleContainerRuntime.build`; **no** `--rosetta` unless user opted in via `runArgs`; deterministic derived tag `adev-{base}:{hash12}` (empty base → `adevcontainer:{hash12}`; no `adevcontainer/features:` prefix); hash includes **`recipeVersion`** epoch — current **`"7"`** (COPY/RUN/`/tmp` identity paths; prior `"6"` = derived LABEL unions base-image + feature lifecycle); reuse when tag exists except nested-`build` `rebuild` ([Dockerfile image](#dockerfile-image-nested-build)). See BuildKit builder lifecycle; install-time env: [gaps — Features install containerEnv](../domain/devcontainer-apple-gaps.md#features-install-containerenv) |
...
| Create | Workspace container from **derived image** with same `--platform`. Features `FROM` the product Dockerfile tag when nested `build`, else config `image`. Create `-u`: explicit `containerUser`, else non-root connection user, else omit when root — see [Connection user](#connection-user-remoteuser--containeruser) |
...
On `AppleContainerRuntime.build` (Features derived image and nested `build` product tag):
...
| Path | Hooks / apply |
|------|----------------|
| Fresh create (`up` bind, `clone` volume, or `rebuild` replacement) | host `initializeCommand` (host path when present; volume/clone-origin `rebuild` with no host workspace: temp guest-config root as above) before create; **Ownership** (`WorkspaceOwnership`) before hooks — eligible named volumes use the pre-start scoped helper when `CHOWN` is unavailable, otherwise the post-start main-container path; `up` fresh create + `rebuild` also verify/fix rootfs workspace parents as capability permits — see [ownership](#named-volumes-ensure--reuse--ownership); then **git credential seeding** (`up` fresh create + `rebuild` replacement; not `clone`) — soft-fail — see [git credential seeding](#git-credential-seeding-create-paths); then `onCreateCommand` → `updateContentCommand` → `postCreateCommand` → `postStartCommand` → **settings+extensions apply** (soft-fail; not gated on `--vscode`); `waitFor` default `updateContentCommand` |
...
| `postAttachCommand` | **CLI attach** on `up`/`clone`/`rebuild`/real `start` after waitFor (not `--vscode`-gated; open success/soft-fail MUST NOT skip). Already-running `start`: **RUNS** only after successful `--vscode` open; **SKIP** + status if no flag or open soft-fails (no status line if absent). Order on apply-commands: apply → open → postAttach. Contract: [`specs/vscode.md`](../../specs/vscode.md) + active [`vscode-customizations-up-clone-rebuild`](../../specs/changes/vscode-customizations-up-clone-rebuild/); archive [`20260814-align-official-lifecycle`](../../specs/changes/archive/20260814-align-official-lifecycle/); open archive: [`specs/changes/archive/20260808-vscode-open-flag/`](../../specs/changes/archive/20260808-vscode-open-flag/); apply archive: [`specs/changes/archive/20260808-vscode-customizations-apply/`](../../specs/changes/archive/20260808-vscode-customizations-apply/) |
