...
- Main managed create keep-alive: empty Feature `entrypoint` list → `--entrypoint /bin/sleep` + image + `infinity`; non-empty → `--entrypoint /bin/sh` `-c` (each Feature entrypoint its own line; wrapper does not `exec` those lines) then `exec /bin/sleep infinity`. Image `ENTRYPOINT`/`CMD` discarded. Ownership/recovery helpers stay sleep-only. `overrideCommand: false` remains blocked. Not a lifecycle hook (`container start` re-runs PID 1). Feature metadata `entrypoint` only — not image `devcontainer.metadata`. Contract: [`specs/features.md`](../../specs/features.md); archive [`20260918-feature-entrypoint-keep-alive`](../../specs/changes/archive/20260918-feature-entrypoint-keep-alive/).
...
**Ownership exec** stays `-u root` (main container and helper). Create `-u` is independent — keep it for Apple attach (typical official image: OCI USER root + metadata `remoteUser` vscode). See [ownership](#named-volumes-ensure--reuse--ownership).
...
Apple named volumes mount **root:root** ([gaps](../domain/devcontainer-apple-gaps.md#named-volume-ownership-rootroot)). `WorkspaceOwnership` makes eligible volumes writable by a non-root connection user before create-path hooks; no-op for root/unset users; never chowns bind mounts, volume `:ro`, or empty targets.

| Path | Behavior |
|------|----------|
| Config `type=volume` mounts | `ensureNamedVolumeMountsWritableByRemoteUser` on `up` / `clone` / `rebuild` (bind and volume mode). Targets only |
| Workspace volume (`adev-*-ws`) | `ensureWorkspaceWritableByRemoteUser` on `clone`; on volume-mode `rebuild` only when the effective connection user **differs** from stamped `remote_user` |
| Workspace parents | `ensureWorkspaceParentsWritableByRemoteUser`: rootfs ancestors of the workspace folder only — never the folder itself (host bind target). Fresh `up` and `rebuild` (both modes); not reuse/start-stopped, bare `start`, or `clone` |

**Helper routing:** `--read-only` is a **rootfs** flag, not volume `:ro`. It selects the same pre-start scoped helper as dropped `CAP_CHOWN` (even with `--cap-add=CHOWN`). Helper create does **not** inherit `--read-only`; writable named volumes still chown in the helper; the main container does not chown them on that path. Otherwise (`CAP_CHOWN` available, no `--read-only`) one post-start `-u root` exec in the main container.

Helper shape: same image/platform; eligible RW named volumes at private targets; root with `ALL` dropped plus `CHOWN` and `DAC_READ_SEARCH`; numeric UID:GID from the shared image; deleted before main start.
...
8. **Ownership + start:** scoped helper (when `--read-only` or dropped `CAP_CHOWN` selects it) chowns writable named volumes then the main container starts; otherwise start first and use the normal ownership path — see [Named volumes / ownership](#named-volumes-ensure--reuse--ownership).
...
| Merge | Feature contributions into effective config before create (`init`, `capAdd`, mounts, lifecycle hooks, Feature `entrypoint` strings for main-create keep-alive wrap — not hooks; runtime `containerEnv` **config wins** on create/exec — separate from install-layer merge above). PATH refs in env expanded on create **and** exec — see PATH expansion |
| Create | Workspace container from **derived image** with same `--platform`. Features `FROM` the product Dockerfile tag when nested `build`, else config `image`. Keep-alive wrap at main create when Feature `entrypoint` strings exist (helpers stay sleep-only). Create `-u`: explicit `containerUser`, else non-root connection user, else omit when root — see [Connection user](#connection-user-remoteuser--containeruser) |
