...
| Workspace parents (bind-mode create paths) | `ensureWorkspaceParentsWritableByRemoteUser` (`parentsOnly` scope): rootfs ancestors of the workspace folder only — never the folder itself (host bind target). `up` fresh create + `rebuild` (both modes, unconditional, incl. volume user-unchanged); no-op for root/unset; not on `up` reuse/start-stopped, bare `start`, or `clone` (existing workspace chown parent walk already covers it) |
...
**Chown script (per target):** `mkdir -p` target → `chown -R user[:user]` target → walk `dirname` parents with **non-recursive** `chown` until a system-top denylist stop (`/`, `/home`, `/Users`, `/var`, `/usr`, `/opt`, `/tmp`, `/root`, `/etc`, `/mnt`, `/media`, `/dev`, `/proc`, `/sys`, `/run`, `/boot`, `/lib`, `/lib64`, `/bin`, `/sbin`). Makes nested home mounts writable for siblings (e.g. `/home/vscode/.local` for `devcontainer-features`) without chowning `/home`. **Never** chown bind-mount host paths. **Parents-only scope variant** (`parentsOnly`, `ensureWorkspaceParentsWritableByRemoteUser`): `mkdir -p` the target and non-recursive ancestor `chown` only — no `chown -R`, never the target itself; same break list (bind-mode create paths; see the table above).
