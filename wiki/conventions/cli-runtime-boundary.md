...
- **postAttach failure** (when it runs): fail the command; **do not** delete/stop the container (fail-keep; contrast create-path delete-on-fail). Open soft-fail alone does not fail the command and must not run postAttach; extensions still run under the `--vscode` flag gate (may complete before open soft-fails).
