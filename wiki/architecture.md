...
| Product binary | `adevcontainer` (PATH). Plugin `bin/dev` is an absolute symlink, not a copy |
...
`install-plugin` is removed. `plugin --install` creates that `bin/dev` symlink; Homebrew target is `$(brew --prefix)/opt/adevcontainer/bin/adevcontainer`, so `brew upgrade adevcontainer` updates the plugin without writing `/usr/local`. `plugin --uninstall` removes only `bin/dev` and `config.toml`. Brew is not sudo; formula `post_install` does not write Apple’s install-root. Apple upgrades wipe the plugin dir ([apple/container#1617](https://github.com/apple/container/issues/1617)); PATH `adevcontainer` survives. Run `sudo adevcontainer plugin --install` once, and again only after that wipe — never `container dev plugin --install` when the layout is missing. Doctor is checks-only and hints that PATH command. Lifecycle does not restage. Contract: [`specs/plugin.md`](../specs/plugin.md) and [`specs/changes/plugin-symlink-install/`](../specs/changes/plugin-symlink-install/). Where the still-active [`install-plugin-command`](../specs/changes/install-plugin-command/) delta says copy or `install-plugin`, the symlink change is the contract. Doctor/identity: [`specs/core.md`](../specs/core.md).
...
| Command | Role |
|---------|------|
| `doctor` | Host/runtime readiness; plugin layout. Checks-only (no restage). Missing plugin: PATH `adevcontainer plugin --install` (`sudo` when needed) — never `container dev plugin --install`. |
| `plugin --install` / `plugin --uninstall` | Install or remove the Apple plugin layout. `--install` writes an absolute symlink at `bin/dev` (not a Mach-O copy). `--uninstall` removes only `bin/dev` and `config.toml`. |
...
| `purge [--name]` | Not an immediate delete on an interactive TTY. Confirmation contract: [`specs/managed-lifecycle.md`](../specs/managed-lifecycle.md). Resource set: container **and** unreferenced candidate volumes (labels `config_volumes` / `workspace_volume` = candidates only; real mounts after target delete decide) **and** config image (`--name` or picker; not binds; not global prune; shared volumes preserved) |
| `inspect [--name]` | Show resolved identity/state (`--name` or picker; no `-w`) |

**Selection:** only `up` takes `-w`/cwd (bind workspace). Lifecycle commands (`start`/`exec`/`stop`/`delete`/`purge`/`rebuild`/`inspect`) resolve managed containers by `--name` or interactive picker — never `-w`.
...
Apple `create --name` is the container **id** and a DNS hostname (official CLI can use labels + a random Docker name; this product cannot). Config `name` is create-name / DNS only. Rebuild-stable resources (`${devcontainerId}`, `*-ws`, Features tags) use workspace/repo basename — never config `name`. Stem / `*-ws` add the identity hash; Features tags add the content hash. Rebuild rename keeps those resources. Detail: [cli-runtime-boundary](conventions/cli-runtime-boundary.md), [gaps — Identity](domain/devcontainer-apple-gaps.md#identity-workaround).

- **Create name** (`create --name` = Apple id / DNS hostname): sanitize(`devcontainer.json` `name`) when non-empty after trim; else sanitize(workspace folder basename) — **volume-mode:** git URL repo basename (not a temp host path). DNS-safe: lowercase; non-`[a-z0-9-]` → `-`; **collapse consecutive hyphens**; trim leading/trailing hyphens; clip ≤63 and re-trim hyphens. **No `adev-` prefix, no identity hash.** Empty after sanitize → structured fail asking for a DNS-safe `name` (no invented `adev-{hash12}`). Example: `"name": "My App"` in folder `foo` → DNS `my-app`.
- **`${devcontainerId}`** (resource stem): `adev-{base}-{hash12}` (empty base → `adev-{hash12}`). **Base is workspace/repo basename, never config `name`.** Same hash material as `*-ws`. Features/config embed it in volume `source` and `containerEnv` (e.g. `${devcontainerId}-shellhistory` → `adev-foo-{hash12}-shellhistory`). Rebuild rename does not rekey stem-keyed volumes. Example: folder `foo` + `"name": "My App"` → stem `adev-foo-{hash12}`.
- **Human base** (`sanitizeBase`, ~20 clip of workspace folder basename / volume-mode git URL repo basename): stem + hashed sidecars. **Never config `name`.** Punctuation-heavy folder names must not yield invalid Apple refs (e.g. folder `C# (.NET)` → `c-net` → Features tag `adev-c-net:{hash12}`, never `adev-c----net:…`).
- **Hashed sidecars** (form unchanged; hash is **not** in the create name):
...
- **Occupancy:**
  ...
  - **`start`:** idle containers are not renamed.

  | Occupant (`up`/`clone`) | Outcome |
  |----------|---------|
  | Same workspace, same create name | `up`: reuse / start-stopped / `config_hash_mismatch`. `clone`: fail-closed. No rename-to-duplicate. |
  | Same workspace, different name (incl. leftover `adev-*`) | Fail + delete-hint. No foreign picker. |
  | Desired name taken by another workspace or unmanaged | TTY: ask whether to change the name, then prompt for the new name (persist into config `name`; clone overlay after populate). No recovery editor or suffix picker. Non-TTY/`--json`: fail + hint. Never delete the occupant. |
...
- Lifecycle hooks: in-container via `container exec` except host `initializeCommand`. Each hook admits **string** | **argv** | **object map** `name → string|argv` (empty `{}` no-op); map entries run **in parallel** (stage succeeds only if every entry exits 0). `waitFor` default `updateContentCommand`. `userEnvProbe` / `shutdownAction` admitted (`stopCompose` fail-closed; explicit `stop` always stops). Detail: [cli-runtime-boundary — Lifecycle](conventions/cli-runtime-boundary.md#lifecycle-execution-hook-matrix). Contract: [`specs/lifecycle-hooks.md`](../specs/lifecycle-hooks.md); archive [`20260814-align-official-lifecycle`](../specs/changes/archive/20260814-align-official-lifecycle/). Matrix:
  ...
  | `postAttachCommand` | **CLI attach** on `up`/`clone`/`rebuild`/real `start` after waitFor (not `--vscode`-gated; open success/soft-fail MUST NOT skip). Already-running `start`: **RUNS** only after successful `--vscode` open; **SKIP** (+ status when any present) if flag absent or open soft-fails. Order on apply-commands: apply → open → postAttach (postAttach still runs if open soft-fails). Non-zero → fail command, **keep** container. Soft-fail apply ≠ postAttach fail-keep. Contract: [`specs/vscode.md`](../specs/vscode.md) + active [`vscode-customizations-up-clone-rebuild`](../specs/changes/vscode-customizations-up-clone-rebuild/); archive [`20260814-align-official-lifecycle`](../specs/changes/archive/20260814-align-official-lifecycle/); open archive: [`specs/changes/archive/20260808-vscode-open-flag/`](../specs/changes/archive/20260808-vscode-open-flag/); apply archive: [`specs/changes/archive/20260808-vscode-customizations-apply/`](../specs/changes/archive/20260808-vscode-customizations-apply/) |
...
- Long-lived main create keeps PID 1 with `/bin/sleep infinity` (Feature `entrypoint` wrap when that list is non-empty). Image `ENTRYPOINT`/`CMD` discarded; `overrideCommand: false` blocked; ownership/recovery helpers stay sleep-only. Not a lifecycle hook. Detail: [cli-runtime-boundary](conventions/cli-runtime-boundary.md). Contract: [`specs/features.md`](../specs/features.md); archive [`20260918-feature-entrypoint-keep-alive`](../specs/changes/archive/20260918-feature-entrypoint-keep-alive/).
...
Not full Dev Containers up/rebuild or IDE-owned customizations parity; volume-mode is product `clone`, not the extension’s clone-in-volume. Contract: [`specs/vscode.md`](../specs/vscode.md) + active [`vscode-customizations-up-clone-rebuild`](../specs/changes/vscode-customizations-up-clone-rebuild/); archive [`20260814-align-official-lifecycle`](../specs/changes/archive/20260814-align-official-lifecycle/); open archive: [`specs/changes/archive/20260808-vscode-open-flag/`](../specs/changes/archive/20260808-vscode-open-flag/); apply archive: [`specs/changes/archive/20260808-vscode-customizations-apply/`](../specs/changes/archive/20260808-vscode-customizations-apply/). Gaps: [devcontainer-apple-gaps.md](domain/devcontainer-apple-gaps.md).
