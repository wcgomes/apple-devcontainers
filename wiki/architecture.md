...
`install-plugin` is removed. `plugin --install` creates that `bin/dev` symlink; Homebrew target is `$(brew --prefix)/opt/adevcontainer/bin/adevcontainer`, so `brew upgrade adevcontainer` updates the plugin without writing `/usr/local`. `plugin --uninstall` removes only `bin/dev` and `config.toml`. Brew is not sudo; formula `post_install` does not write Apple’s install-root. Apple upgrades wipe the plugin dir ([apple/container#1617](https://github.com/apple/container/issues/1617)); PATH `adevcontainer` survives. Run `sudo adevcontainer plugin --install` once, and again only after that wipe — never `container dev plugin --install` when the layout is missing. Doctor is checks-only and hints that PATH command. Lifecycle does not restage. Contract: [`specs/plugin.md`](../specs/plugin.md). Archive: [`20260925-plugin-symlink-install`](../specs/changes/archive/20260925-plugin-symlink-install/), [`20260925-install-plugin-command`](../specs/changes/archive/20260925-install-plugin-command/). Doctor/identity: [`specs/core.md`](../specs/core.md).
