...
Apple upgrades wipe that plugin dir ([apple/container#1617](https://github.com/apple/container/issues/1617)). PATH `adevcontainer` survives. Restage with PATH `adevcontainer install-plugin` (`sudo` when needed) — never `container dev install-plugin` when missing. Doctor is checks-only. Lifecycle does not restage. Contract: [`specs/plugin.md`](../specs/plugin.md); active [`install-plugin-command`](../specs/changes/install-plugin-command/). Doctor/identity: [`specs/core.md`](../specs/core.md).
...
| Command | Role |
|---------|------|
| `doctor` | Host/runtime readiness; plugin layout. Checks-only (no restage). Missing plugin: PATH `adevcontainer install-plugin` (`sudo` when needed) — never `container dev install-plugin`. |
| `install-plugin` | Restage Apple plugin from PATH `adevcontainer` (`sudo` when needed). |
