...
| Named volume ownership | Engine/user mapping varies; often usable by non-root | Apple named volumes mount **root:root**. `WorkspaceOwnership` chowns eligible volumes in the main container when `CHOWN` is available or through a scoped ephemeral helper when `runArgs` removes it; rootfs parents are fixed when possible or checked with explicit failure. Bind targets remain untouched — see [Named volume ownership](#named-volume-ownership-rootroot) |
...
**Product:** `WorkspaceOwnership` uses the normal post-start main-container path while `CHOWN` remains available. When it is unavailable, a pre-start scoped helper handles only read-write named volumes without changing the main container's requested capabilities. The helper cannot alter private main-container rootfs parents, so those are checked after start and fail with explicit `CAP_CHOWN` remediation when inaccessible. Ownership is mandatory; real Apple-runtime validation of the helper path remains pending. Detailed targets, UID/GID handling, cleanup, and failure behavior: [cli-runtime-boundary — Named volumes / ownership](../conventions/cli-runtime-boundary.md#named-volumes-ensure--reuse--ownership). Contract: [`specs/core.md`](../../specs/core.md).
