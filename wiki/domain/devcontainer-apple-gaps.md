...
| Dockerfile / nested `build` | Official CLI builds from Dockerfile | Nested `build` xor `image` is representable (Compose and top-level dockerfile selectors stay blocked). Rebuild COPY in volume-mode is best-effort for in-directory context. Detail: [cli-runtime-boundary — Dockerfile image](../conventions/cli-runtime-boundary.md#dockerfile-image-nested-build). Contract: [`specs/core.md`](../../specs/core.md) |
...
| Features | OCI + local path feature packages + image build | **Partial** (OCI + local path on bind `up`/`rebuild`, `clone`, and volume `rebuild` — [cli-runtime-boundary](../conventions/cli-runtime-boundary.md#features-runner)); **warn-skip** `docker-outside-of-docker`, `docker-in-docker`, `docker-from-docker` and privileged/securityOpt metadata ([0003](../decisions/0003-warn-skip-apple-incompatibles.md)); native arm64 build (no Rosetta by default) |
...
| Default process | Official CLI: Feature `entrypoint` wrap then keep-alive sleep | Main create: sleep-only when Feature `entrypoint` list is empty; else wrap then `exec /bin/sleep infinity`. Image `ENTRYPOINT`/`CMD` discarded; helpers sleep-only. Detail: [cli-runtime-boundary](../conventions/cli-runtime-boundary.md) |
...
| Named volume copy-up | Docker/Podman copy preexisting image content at the mount target into a new empty named volume | Apple mounts the new volume empty. Fresh bind-mode `up` emulates copy-up only for config volumes created by that attempt, through a same-image helper before the main container and its in-container lifecycle hooks. Existing volumes are never overwritten; a newly created volume targeting `/` is rejected. Detail: [cli-runtime-boundary](../conventions/cli-runtime-boundary.md#image-copy-up-for-new-config-volumes) |
| Named volume ownership | Engine/user mapping varies; often usable by non-root | Apple named volumes mount **root:root**. `WorkspaceOwnership` chowns eligible volumes in the main container, or through a scoped helper when `--read-only` (rootfs, not volume `:ro`) or dropped `CAP_CHOWN` selects it. Unmodifiable but traversable ancestors (Apple EPERM on `/workspaces` even as root) do not fail `up`. Bind targets remain untouched — see [Named volume ownership](#named-volume-ownership-rootroot) |
...
| Auth for git | **HTTPS:** bind-mode create paths (`up` fresh create; `rebuild` bind) seed a username-agnostic POSIX-sh helper in the effective connection user's HOME before create-path hooks — HOME is resolved from the effective user, the helper is 0700, its private store is 0600, and it is appended with `git config --global --add credential.helper`; unique pathless entries are approved via stdin, and helper `get` matches protocol+host while ignoring the queried username and returns that username (or the stored username when the query has none) with the stored password, covering sibling repos. Host acquisition uses `git credential fill` (`ADEVCONTAINER_GIT_TOKEN` / `gh auth token` fallbacks); seeded credential material enters the guest via stdin only, never argv or env. Silent skip: host git missing / no repo or remotes / fill nil (public stays anonymous). Soft-fail warn. **Not seeded:** SSH remotes; non-create paths (`up` reuse, `up` start-stopped, bare `start`) | **SSH:** `SSH_AUTH_SOCK` + `create --ssh`. **HTTPS:** clone's separate populate uses host `git credential fill` one-shot → guest `credential.helper store` + approve. No GCM-in-guest; no host `~/.git-credentials` mount; no PAT CLI primary UX |
...
**Product:** `WorkspaceOwnership` chowns in the main container after start when `CAP_CHOWN` is available and `runArgs` has no `--read-only`. `--read-only` is a rootfs flag (not volume `:ro`) and selects the same pre-start scoped helper as dropped `CAP_CHOWN`; the helper does not inherit `--read-only` and still chowns writable named volumes. Some Apple rootfs inodes (`/workspaces`) reject `chown` with EPERM even as root; unmodifiable but traversable ancestors do not fail `up`. Recursive target `chown -R` stays strict. The helper cannot repair the main private rootfs; on the helper path, parents are probed as the connection user. Create `-u` for metadata `remoteUser` (typical OCI USER root + metadata vscode) stays; ownership exec stays `-u root`. Real Apple-runtime validation of the ownership helper path remains pending. Detail: [cli-runtime-boundary — Named volumes / ownership](../conventions/cli-runtime-boundary.md#named-volumes-ensure--reuse--ownership). Contract: [`specs/core.md`](../../specs/core.md).
...
- After atomic `mv`, `chown` the in-volume stamped `devcontainer.json` to the parent-directory owner, then `chmod 644` — connection-user **writable**, not merely readable, not world-writable. Host session stays `0700`/`0600`.
...
- Volume named/retry rebuild must **not** re-open the interactive picker once a container is selected; same id via pinned `--name` / `selectedOverride` (bind resume already did this).
...
- **Source selector**: nested `build` xor `image` is representable. Compose and top-level `dockerFile`/`dockerfile`/`context` stay blocked. Detail: [cli-runtime-boundary — Dockerfile image](../conventions/cli-runtime-boundary.md#dockerfile-image-nested-build). Contract: [`specs/core.md`](../../specs/core.md).
